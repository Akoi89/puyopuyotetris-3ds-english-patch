"""Independent read-back of the 1.0.15 base builds against the shipped 1.0.14.

    python _issue1_verify.py [en|jp]     (default en)

Base: TMD version 1.0.15; contents 1 and 2 identical to the previous CIA; content 0 identical outside
the romfs and its hash fields (code and exefs untouched); romfs region == new_115[_jpv].romfs; the
extracted romfs tree differs from the previous build's tree in exactly title/title.narc and the adventure text
and scene scripts changed by the width pass.
...
MU = 0x200
if ED == 'en':
    OLD = os.path.join(R, r'Final\_CURRENT\PuyoPuyoTetris-EN-voices-1.0.14.cia')
    NEW = os.path.join(R, r'Final\_new\PuyoPuyoTetris-EN-voices-1.0.15.cia')
    ROMFS = 'new_115.romfs'
else:
    OLD = os.path.join(R, r'Final\_CURRENT\PuyoPuyoTetris-JP-voices-1.0.14.cia')
    NEW = os.path.join(R, r'Final\_new\PuyoPuyoTetris-JP-voices-1.0.15.cia')
    ROMFS = 'new_115_jpv.romfs'
STEMS = ['chapter%02d' % i for i in range(1, 8)] + ['general']
EXPECT = {os.path.join('title', 'title.narc')}
for _s in STEMS:
    EXPECT.add(os.path.join('tenp', 'text', 'adventure', '%sJapanese.mtx' % _s))
    EXPECT.add(os.path.join('script', 'adventure', _s, 'manzai_script_%s.narc' % _s))
fails = []
...
check(new.version() == (1, 0, 15), 'new base TMD version is 1.0.15')
...
check(set(d) == EXPECT, 'romfs trees differ in exactly the title stamp, the 8 adventure text files and the 8 scene scripts (got %d: %s)' % (len(d), sorted(d)))
...
# the version stamp, decoded out of the CIA's own title.narc, not out of patch/
import narc as _narc, labels as _labels
from PIL import Image as _Image
_m = _narc.read(os.path.join('_v_new_tree', 'title', 'title.narc'))
for _mi, _b in enumerate(_m['members']):
    if _b[:4] == b'CTPK':
        for _e in _labels.ctpk_entries(_b):
            if _e['name'] == 'title_tds2_d8888':
                _img = _labels.dec(_b[_e['off']:_e['off'] + _e['size']], _e['w'], _e['h'], _e['fmt'])
                _img.crop((_e['w'] - 470, _e['h'] // 2 - 40, _e['w'] - 120, _e['h'] // 2 + 40)).save(
                    '_v_stamp_readback_%s.png' % ED)
                print('version stamp read back out of the built CIA -> _v_stamp_readback_%s.png' % ED)
...
# ---------------- DLC ----------------
# 1.0.15 does not touch the DLC; check only that the shipped 0.2.8 CIAs are still the published bytes.
