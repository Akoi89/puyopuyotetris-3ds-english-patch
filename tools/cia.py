...
class Cia(object):
    ...
    def write(self, path, replace=None, version=None):
        ...
        if version is not None:
            maj, mi, mic = version
            # The title version is 16 bits: major 6, minor 6, micro 4. An out-of-range
            # part silently carries into the next field -- 1.0.99 packed as 1.6.3 on a
            # 2026-09-03 probe build and the CIA reported a version nobody asked for.
            if not (0 <= maj < 64 and 0 <= mi < 64 and 0 <= mic < 16):
                raise SystemExit(
                    'version %d.%d.%d does not fit the 3DS title version '
                    '(major/minor 0-63, micro 0-15)' % (maj, mi, mic))
            v = (maj << 10) | (mi << 4) | mic
