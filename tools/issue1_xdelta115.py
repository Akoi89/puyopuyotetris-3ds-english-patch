# -*- coding: utf-8 -*-
"""Re-encode the release xdelta patches for 1.0.15 / DLC 0.2.8 (the DLC patches are unchanged; pass a name filter to skip them) (both editions), the proven way:
...
region is random, then applied back on the real source plus two randomly perturbed copies.
    ...
    python _issue1_xdelta115.py en    -> work/rhdn_xdelta_v7/        (EN voices 1.0.15 + DLC 0.2.8)
    python _issue1_xdelta115.py jp    -> work/rhdn_xdelta_jpv_115/   (JP voices 1.0.15 + DLC JP voices 0.2.8)
"""
...
if ED == 'en':
    OUT = os.path.join(R, 'work', 'rhdn_xdelta_v7')
    JOBS = [dict(name='PuyoPuyoTetris-EN-voices-1.0.15.xdelta', src=BASE_SRC,
                 tgt=os.path.join(R, r'Final\_new\PuyoPuyoTetris-EN-voices-1.0.15.cia'), scrub=(0, 0x4000), volatile=(0x1010, 0x2C)),
    ...
    OUT = os.path.join(R, 'work', 'rhdn_xdelta_jpv_115')
    JOBS = [dict(name='PuyoPuyoTetris-JP-voices-1.0.15.xdelta', src=BASE_SRC,
                 tgt=os.path.join(R, r'Final\_new\PuyoPuyoTetris-JP-voices-1.0.15.cia'), scrub=(0, 0x4000), volatile=(0x1010, 0x2C)),
