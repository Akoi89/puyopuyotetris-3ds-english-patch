# -*- coding: utf-8 -*-
"""Build and verify the 1.0.15 zips. The DLC is unchanged at 0.2.8, so its xdelta and CIA are reused
as published; only the base patch, the two LayeredFS zips and the two bundles are rebuilt.

    python _issue1_zips115.py en      Base-xdelta.zip, LayeredFS.zip, bundle 1.0.15
    python _issue1_zips115.py jp      JP-voices-LayeredFS.zip, JP bundle 1.0.15   (run after en)

The LayeredFS entry list is taken from the overlay tree itself rather than from the previous zip, because
1.0.15 adds fourteen files to it (the eight adventure text tables and six more scene scripts). Every entry is checked to be either one of the previous
...
NEW_PATHS = {PFX + 'tenp/text/adventure/%sJapanese.mtx' % s for s in STEMS}
NEW_PATHS |= {PFX + 'script/adventure/%s/manzai_script_%s.narc' % (s, s) for s in STEMS}
...
def layeredfs_entries(readme, drop_sound):
    """Every file in the 1.0.15 overlay, in the previous zip's order, with the new files appended."""
...
xd3 = zipfile.ZipFile(r'Final\_CURRENT\PuyoPuyoTetris-xdelta-patches-1.0.14.zip').read('xdelta3.exe')

if ED == 'en':
    RD = os.path.join(W, 'readmes_1015')
    EN_SHA = fsha(r'Final\_new\PuyoPuyoTetris-EN-voices-1.0.15.cia')
    DLC_SHA = fsha(r'Final\_CURRENT\PuyoPuyoTetris-DLC-0.2.8.cia')
    base_xd = open(os.path.join(W, r'rhdn_xdelta_v7\PuyoPuyoTetris-EN-voices-1.0.15.xdelta'), 'rb').read()
    ...
    retire(os.path.join(W, 'PuyoPuyoTetris-Base-xdelta.zip'), r'Final\_superseded\PuyoPuyoTetris-Base-xdelta-1.0.14.zip')
    zwrite(os.path.join(W, 'PuyoPuyoTetris-Base-xdelta.zip'), [('README.txt', rd), ('PuyoPuyoTetris-EN-voices-1.0.15.xdelta', base_xd)])
    ...
    retire(os.path.join(W, 'PuyoPuyoTetris-LayeredFS.zip'), r'Final\_superseded\PuyoPuyoTetris-LayeredFS-1.0.14.zip')
    ...
    bundle = r'Final\_new\PuyoPuyoTetris-xdelta-patches-1.0.15.zip'
    zwrite(bundle, [('README.md', rh), ('PuyoPuyoTetris-EN-voices-1.0.15.xdelta', base_xd),
                    ('PuyoPuyoTetris-DLC-0.2.8.xdelta', dlc_xd), ('xdelta3.exe', xd3)])
    shutil.copyfile(os.path.join(RD, 'README_rhdn.md'), r'Final\_new\rhdn_bundle_README_1.0.15.md')
    verify_bundle(bundle, [(BASE_SRC, 'PuyoPuyoTetris-EN-voices-1.0.15.xdelta', EN_SHA),
    ...
    RD = os.path.join(W, 'readmes_jpv_115')
    JP_SHA = fsha(r'Final\_new\PuyoPuyoTetris-JP-voices-1.0.15.cia')
    JP_DLC_SHA = fsha(r'Final\_CURRENT\PuyoPuyoTetris-DLC-JP-voices-0.2.8.cia')
    base_xd = open(os.path.join(W, r'rhdn_xdelta_jpv_115\PuyoPuyoTetris-JP-voices-1.0.15.xdelta'), 'rb').read()
    ...
    retire(lfs, r'Final\_superseded\PuyoPuyoTetris-JP-voices-LayeredFS-1.0.14.zip')
    zwrite(lfs, entries)
    ...
    bundle = r'Final\_new\PuyoPuyoTetris-JP-voices-xdelta-patches-1.0.15.zip'
    zwrite(bundle, [('README.md', rh), ('PuyoPuyoTetris-JP-voices-1.0.15.xdelta', base_xd),
                    ('PuyoPuyoTetris-DLC-JP-voices-0.2.8.xdelta', dlc_xd), ('xdelta3.exe', xd3)])
    open(r'Final\_new\rhdn_bundle_README_JP-voices_1.0.15.md', 'wb').write(rh)
    verify_bundle(bundle, [(BASE_SRC, 'PuyoPuyoTetris-JP-voices-1.0.15.xdelta', JP_SHA),
